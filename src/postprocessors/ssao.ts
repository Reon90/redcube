...
export class SSAO extends PostProcessor {
    ...
    attachUniform(program) {
        gl.uniform1i(
            gl.getUniformLocation(program, 'ssao'),
            this.ssaoTexture.index
        );
    }
    ...
    postProcessing(PP) {
        ...
        const cameraProps =
            this.camera.props.perspective || this.camera.props.orthographic;
        gl.uniform1i(
            gl.getUniformLocation(this.ssaoProgram, 'normBuff'),
            PP.normalTexture.index
        );
        gl.uniform1i(
            gl.getUniformLocation(this.ssaoProgram, 'depthBuff'),
            PP.depthTexture.index
        );
        gl.uniform1i(
            gl.getUniformLocation(this.ssaoProgram, 'noice'),
            this.noice.index
        );
        gl.uniform2f(
        ...
        gl.uniform1f(
            gl.getUniformLocation(this.ssaoProgram, 'zFar'),
            cameraProps.zfar
        );
        gl.uniform1f(
            gl.getUniformLocation(this.ssaoProgram, 'zNear'),
            cameraProps.znear
        );
        gl.uniformMatrix4fv(
            gl.getUniformLocation(this.ssaoProgram, 'proj'),
            false,
            this.camera.projection.elements
        );
        gl.uniformMatrix4fv(
            gl.getUniformLocation(this.ssaoProgram, 'view'),
            false,
        ...
        gl.uniform3fv(
            gl.getUniformLocation(this.ssaoProgram, 'kernels'),
            this.kernels
        );
        ...
        gl.uniform1i(
            gl.getUniformLocation(this.ssaoBlurProgram, 'uTexture'),
            this.ssaoTexture.index
        );
        gl.uniform2f(
            gl.getUniformLocation(this.ssaoBlurProgram, 'denom'),
            1,
            0
        );
        gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
    ...
    buildNoice(pp) {
        ...
        for (let i = 0; i < noiceSize * noiceSize; i++) {
            const v = new Vector3([
                ...
                random(0, 1) * 2.0 - 1.0,
                0.1
            ]); // Z is 0.1 because surface is not flat
            /* eslint-disable */
