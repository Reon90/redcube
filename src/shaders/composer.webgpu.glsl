...
void main() {
    ...
    #ifdef BLOOM
        c += texture(sampler2D(bloom, baseSampler).rgb;
    #endif
    #ifdef SSAO
        c *= texture(sampler2D(ssao, baseSampler).r;
    #endif
    #ifdef LIGHT
        c += texture(sampler2D(light, baseSampler).r;
    #endif
    #ifdef SCATTERING
        c = texture(sampler2D(scattering, baseSampler).rgb;
        c += texture(sampler2D(cpec, baseSampler).rgb;
    #endif
